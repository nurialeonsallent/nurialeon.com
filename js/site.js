...
{
    window.location.hash = "gotoContactForm";
    contactForm.Nombre.focus();
}

function formButtonClicked(name)
{
    formMethod = name;
}
...
${text}`;
    ...
    const captchaOK = triggerCaptcha("2 + 2 = ?", ["4", "cuatro"]);

    if (captchaOK)
    {
        contactForm.action = 'https://formspree.io/f/' + (() =>
        {
            switch (formName)
            {
                case 'opinion':
                ...
                case 'contact':
                    return 'xvowdevg';
                default:
                    alert("Ha ocurrido un error, no se puede enviar el formulario");
                    return null;
            }
        })();
    }
...
function triggerCaptcha(question, expectedResponses)
{
    const response = prompt("¿Eres un robot?\nPor favor resuelve la siguiente operación:\n" + question);

    if (!expectedResponses.some(r => r.trim().toLowerCase() === response.trim().toLowerCase()))
    {
